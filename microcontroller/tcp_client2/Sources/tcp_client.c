...
static err_t connection_callback(void *arg, struct tcp_pcb *tpcb, err_t err) {
	const *message = "HEAD /darkmode/toggle HTTP/1.0\r\n\r\n";
...
err_t create_tcp_connection() {
	...
	ipaddr_aton(REMOTE_IP, &remote_ip);
	ip_addr_set(&local_ip, &netif.ip_addr);

	/* create new protocol control block (pcb) */
	pcb = tcp_new();
	error = tcp_bind(pcb, &local_ip, local_port);
	if(error)
		return error;

	error = tcp_connect(pcb, &remote_ip, REMOTE_PORT, connection_callback);
	if(error)
		return error;
